...
- `items`
- `total`
- `page`：页码模式为正整数；旧游标/offset 模式为 `null`，避免伪造无法推导的页码
- `page_size`
- `total_pages`
- 原响应已有的 `next_cursor` 暂时保留；旧游标模式继续返回真实值，页码模式返回 `null`

两种模式都返回当前项目、权限范围和筛选条件下的真实 `total`，不是当前已加载集合的长度；`total_pages` 按响应 `page_size` 计算。空集合的 `total_pages` 为 0，前端显示第 0/0 页且禁用翻页操作。
