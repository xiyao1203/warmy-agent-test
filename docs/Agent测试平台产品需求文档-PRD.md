...
- 人工表单、Agent 生成、文件导入、失败回归和执行引擎必须读写同一个专业用例契约，禁止维护聊天专用或 Worker 专用的第二套格式。
- 专业用例至少包含可验证目标、输入数据、前置条件、有序操作步骤、逐步测试数据与预期结果、整体断言、评分、安全、证据和执行设置。
- Agent 生成内容先保存为可编辑草稿；校验通过后才能标记就绪，发布的数据集版本和运行快照不可变。
...
- 核心列表必须提供完成当前决策所需的摘要，而不是只展示名称和 ID：资产列表显示版本/发布/覆盖信息，运行与治理列表显示资源引用、状态、质量、成本、风险、阻塞项和最近活动。
- 项目、Agent、数据集、计划、运行、环境、评分器、实验、安全、审核和门禁之间使用可访问的站内资源引用导航；不可信或站外 `href` 只能显示文本，不能直接生成链接。
...
```json
{
  ...
  "assertions": [{ "type": "equals", "path": "refused", "value": true }],
  "scorers": [],
  "security_policies": [],
...
- 平台提供分区表单人工新增和编辑以上字段，步骤支持添加、复制、删除和排序；高级 JSON 只承载受大小限制的扩展字段，不代替标准字段。
- 草稿可以校验、复制和单用例 AI 试运行；就绪是进入发布版本的质量状态，发布后只能通过新数据集版本继续编辑。
- 单用例试运行必须选择已发布 Agent 版本和执行环境并显式确认外部影响。平台固化无秘密的专业用例快照，结果通过来源用例关联回测试资产，默认不混入正式计划统计。
- 导入、导出、Test Agent 生成、计划编译、API/Browser/Codex Explore 执行必须保持字段语义、步骤顺序和来源信息一致。
...
Run 以 `run_type` 区分正式计划运行和 `case_trial`。正式运行引用不可变测试计划版本；单用例试运行引用来源用例并保存递归脱敏的完整 `case_spec_snapshot`，Worker 只消费快照，不回读可变业务表。确定性浏览器用例必须同时保留测试工程师可读步骤与结构化自动化动作；同一试运行幂等键只复用完全相同的用例快照、Agent 版本和环境，正式 Run 的同一幂等键只复用完全相同的项目与测试计划版本。并发请求最多创建一个 Run，失败请求不得因数据库事务失效返回 500。
...
- 重试必须保留原始失败记录。
- 任务需要支持取消和超时。
- 运行依赖缺失、外部工具失败或返回无效结果时必须明确失败，不得返回预设结果、假成功或自动跳过。
- 报告、测试 Agent 会话和失败回归用例必须来自持久化业务数据，不得使用进程内临时事实源。
...
- 用户显式配置的网络 Mock/故障注入属于测试环境能力；生产服务不得自动回退到 Mock 扫描器、Mock 模型或演示数据。
...
- 测试工程师可以通过分区表单维护输入、准备、步骤/逐步数据与预期、断言、评分、安全、证据和执行设置；Agent 生成用例可用同一表单复核编辑。
- 全部核心业务列表展示决策摘要和可访问的跨资源引用，在 1280px 桌面及 390px 窄屏不产生页面级横向滚动。
...
- 专业用例可从草稿完成校验、标记就绪和单用例 AI 试运行；三种执行模式消费同一不可变专业用例快照。
