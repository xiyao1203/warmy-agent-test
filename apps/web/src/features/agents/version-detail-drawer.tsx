...
export function VersionDetailDrawer({
  ...
  const modelParams = config.model_params as
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* 抽屉内容 */}
      <div className="relative z-10 w-full max-w-md overflow-y-auto border-l border-[var(--hairline)] bg-[var(--surface)] p-6">
        {/* 头部 */}
        ...
        <div className="mt-6 space-y-6">
          ...
          <Section title="Agent 配置" icon={<Cpu className="size-4" />}>
            <Field
              label="API 地址"
              value={String(config.api_url ?? "未设置")}
            />
          ...
          >
            <Field
              ...
              value={String(config.system_prompt_version ?? "未设置")}
            />
            <Field
              label="知识库版本"
              value={String(config.knowledge_version ?? "未设置")}
            />
            <Field
              label="Adapter 版本"
              value={String(config.adapter_version ?? "未设置")}
            />
          ...
          <Section title="执行限制" icon={<Clock className="size-4" />}>
            ...
            <Field
              label="最大步数"
              value={config.max_steps ? String(config.max_steps) : "无限制"}
            />
            <Field
              label="成本限制"
              value={config.cost_limit ? `¥${config.cost_limit}` : "无限制"}
            />
          </Section>
