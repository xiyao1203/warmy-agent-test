...
export function TestCaseEditor({
  ...
  const [activeSection, setActiveSection] = useState<"basic" | "input" | "output" | "scoring" | "advanced">("basic");
  ...
  const [priority, setPriority] = useState<string>(caseItem?.priority ?? "");
  const [riskLevel, setRiskLevel] = useState<string>(caseItem?.risk_level ?? "");
  const [difficulty, setDifficulty] = useState<string>(caseItem?.difficulty ?? "");
  const [testGroup, setTestGroup] = useState<string>(caseItem?.test_group ?? "");
  ...
  async function submit() {
    try {
      ...
      const parsedAssertions = JSON.parse(assertions) as Array<Record<string, unknown>>;
      const parsedScorers = JSON.parse(scorers) as Array<Record<string, unknown>>;
      const parsedSecurityPolicies = JSON.parse(securityPolicies) as Array<Record<string, unknown>>;
      const parsedInitialState = JSON.parse(initialState) as Record<string, unknown>;
