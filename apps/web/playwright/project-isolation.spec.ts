...
test.describe("project isolation", () => {
  test("super admin can create projects and assign developer to one", async ({ page }) => {
    await page.goto("/");
  ...
  test("developer assigned to project A cannot access project B", async ({ browser }) => {
    ...
    const currentProjectId = currentUrl.match(/\/projects\/([^/]+)\/overview/)?.[1];

    await devPage.goto("/projects/00000000-0000-0000-0000-000000000000/overview");
    await expect(
      devPage.getByText(/项目不存在或你无权访问/),
    ).toBeVisible();
