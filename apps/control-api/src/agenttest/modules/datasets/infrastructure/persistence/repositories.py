...
class SqlAlchemyDatasetRepository:
    ...
    async def list_page_by_project(
        ...
        return PageResult(
            items=[_to_dataset(model) for model in models],
            total=total,
...
def _to_test_case(model: TestCaseModel) -> TestCase:
    ...
    return TestCase(
        ...
        case_status=TestCaseStatus(model.case_status),
        template=TestCaseTemplate(model.template),
        case_type=TestCaseType(model.case_type),
        automation_status=AutomationStatus(model.automation_status),
        source=TestCaseSource(model.source),
        source_ref=model.source_ref,
        component=model.component,
        requirement_refs=model.requirement_refs,
        owner_id=UserId(model.owner_id) if model.owner_id else None,
        input=model.input,
        ...
        data_bindings=model.data_bindings,
        steps=model.steps,
        expected_outcome=model.expected_outcome,
        security_policies=model.security_policies,
        artifact_requirements=model.artifact_requirements,
