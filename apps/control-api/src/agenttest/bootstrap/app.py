...
from agenttest.modules.identity.infrastructure.security.password import (
    Argon2PasswordHasher,
)
from agenttest.modules.identity.infrastructure.security.session import (
    CsrfValidator,
    SystemClock,
)
...
from agenttest.shared.infrastructure.persistence.unit_of_work import SqlAlchemyUnitOfWork
...
def build_user_settings_dependencies(settings: Settings) -> UserSettingsApiDependencies:
    ...
    users = SqlAlchemyUserRepository(session_factory)
    sessions = SqlAlchemySessionRepository(session_factory)
    settings_repo = SqlAlchemyUserSettingsRepository(session_factory)
    clock = SystemClock()
    return UserSettingsApiDependencies(
        current_user=CurrentUserQuery(users=users, sessions=sessions, clock=clock),
        get_settings=GetUserSettingsHandler(repository=settings_repo),
        update_settings=UpdateUserSettingsHandler(repository=settings_repo),
        uow_factory=lambda: SqlAlchemyUnitOfWork(session_factory),
    )
...
def build_feedback_dependencies(settings: Settings) -> FeedbackApiDependencies:
    ...
    users = SqlAlchemyUserRepository(session_factory)
    sessions = SqlAlchemySessionRepository(session_factory)
    feedback_repo = SqlAlchemyFeedbackRepository(session_factory)
    clock = SystemClock()
    return FeedbackApiDependencies(
        current_user=CurrentUserQuery(users=users, sessions=sessions, clock=clock),
        create_feedback=CreateFeedbackHandler(repository=feedback_repo),
        uow_factory=lambda: SqlAlchemyUnitOfWork(session_factory),
    )
...
def build_agent_dependencies(settings: Settings) -> AgentApiDependencies:
    ...
    agent_versions = SqlAlchemyAgentVersionRepository(session_factory)
    return AgentApiDependencies(
        create_agent=CreateAgentHandler(agents=agents),
        update_agent=UpdateAgentHandler(agents=agents),
        get_agent=GetAgentHandler(agents=agents),
        list_agents=ListAgentsHandler(agents=agents),
        create_agent_version=CreateAgentVersionHandler(agents=agents, versions=agent_versions),
        update_agent_version=UpdateAgentVersionHandler(versions=agent_versions),
        publish_agent_version=PublishAgentVersionHandler(versions=agent_versions),
        get_agent_version=GetAgentVersionHandler(versions=agent_versions),
        list_agent_versions=ListAgentVersionsHandler(versions=agent_versions),
    )
...
def build_dataset_dependencies(settings: Settings) -> DatasetApiDependencies:
    ...
    dataset_versions = SqlAlchemyDatasetVersionRepository(session_factory)
    test_cases = SqlAlchemyTestCaseRepository(session_factory)
    return DatasetApiDependencies(
        create_dataset=CreateDatasetHandler(datasets=datasets),
        update_dataset=UpdateDatasetHandler(datasets=datasets),
        get_dataset=GetDatasetHandler(datasets=datasets),
        list_datasets=ListDatasetsHandler(datasets=datasets),
        create_dataset_version=CreateDatasetVersionHandler(
            datasets=datasets, versions=dataset_versions
        ),
        publish_dataset_version=PublishDatasetVersionHandler(versions=dataset_versions),
        get_dataset_version=GetDatasetVersionHandler(versions=dataset_versions),
        list_dataset_versions=ListDatasetVersionsHandler(versions=dataset_versions),
        add_test_case=AddTestCaseHandler(test_cases=test_cases),
        update_test_case=UpdateTestCaseHandler(test_cases=test_cases),
        delete_test_case=DeleteTestCaseHandler(test_cases=test_cases),
        get_test_case=GetTestCaseHandler(test_cases=test_cases),
        list_test_cases=ListTestCasesHandler(test_cases=test_cases),
...
def build_environment_dependencies(settings: Settings) -> EnvironmentApiDependencies:
    ...
    return EnvironmentApiDependencies(
        create_template=CreateEnvironmentTemplateHandler(templates=templates),
        update_template=UpdateEnvironmentTemplateHandler(templates=templates),
        delete_template=DeleteEnvironmentTemplateHandler(templates=templates),
        get_template=GetEnvironmentTemplateHandler(templates=templates),
        list_templates=ListEnvironmentTemplatesHandler(templates=templates),
    )


def build_test_plan_dependencies(settings: Settings):
    ...
    return TestPlanApiDependencies(
        create_plan=CreateTestPlanHandler(plans=plans),
        update_plan=UpdateTestPlanHandler(plans=plans),
        get_plan=GetTestPlanHandler(plans=plans),
        list_plans=ListTestPlansHandler(plans=plans),
    )


def build_run_dependencies(settings: Settings):
    from agenttest.modules.runs.api.router import RunApiDependencies
    from agenttest.modules.runs.application.commands import CreateRunHandler
    from agenttest.modules.runs.application.queries import GetRunHandler, ListRunsHandler
    ...
    return RunApiDependencies(
        create_run=CreateRunHandler(runs=runs),
        get_run=GetRunHandler(runs=runs),
        list_runs=ListRunsHandler(runs=runs),
    )
from datetime import timedelta
from pathlib import Path
